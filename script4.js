...
async function obtenerDatos (url) {
    try {
        const response = await fetch(url); // Haemos el llamado a la url que le vamos a pasar omo parametro
        return response.json(); // Lo transformamos en JSON
        // y haemos que esta funion retorne ese valor
    } 
   catch (error) {
...
async function verDatos () {
    const data = await obtenerDatos('https://rickandmortyapi.com/api/character/?page=7'); // le pasamos el url omo parametro
    const data2 = await obtenerDatos('https://rickandmortyapi.com/api/character/?page=8'); // Lo almaenamos en "data"
